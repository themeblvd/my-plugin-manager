...
if ( ! class_exists( '_My_Plugin_Manager' ) ) {
	...
	class _My_Plugin_Manager {
		...
		public function __construct( $plugins, $args = array() ) {
			...
			// Hook Ajax requests.
			add_action( 'wp_ajax_my_namespace-row-refresh', array( $this, 'row_refresh' ) );
		...
		private function set_plugins( $plugins ) {
			...
			/**
			 * Include _My_Plugins class to store
			 * plugin data.
			 */
			require_once( $this->package['directory'] . '/class-my-plugins.php' );

			// Setup plugins.
			$this->plugins = new _My_Plugins( $plugins, $this );
		...
		private function set_args( $args = array() ) {
			...
			$this->args = wp_parse_args( $args, array(
				'page_title'           => __( 'Suggested Plugins', 'my-text-domain' ),
				// translators: 1: name of theme
				'views_title'          => sprintf( __( 'Suggested by %s', 'my-text-domain' ), $this->package['name'] ),
				// translators: 1: name of theme
				'tab_title'            => sprintf( __( 'Suggested by %s', 'my-text-domain' ), $this->package['name'] ),
				// translators: 1: name of theme
				'extended_title'       => sprintf( __( 'Suggested Plugins by %s', 'my-text-domain' ), $this->package['name'] ),
				'menu_title'           => '', // Takes on page_title, when left blank.
				'parent_slug'          => 'themes.php',
				'menu_slug'            => 'suggested-plugins',
				'capability'           => 'install_plugins',
				'nag_action'           => __( 'Manage suggested plugins', 'my-text-domain' ),
				'nag_dismiss'          => __( 'Dismiss this notice', 'my-text-domain' ),
				// translators: 1: name of theme
				'nag_update'           => __( 'Not all of your active, suggested plugins are compatible with %s.', 'my-text-domain' ),
				// translators: 1: name of theme, 2: number of suggested plugins
				'nag_install_single'   => __( '%1$s suggests installing %2$s plugin.', 'my-text-domain' ),
				// translators: 1: name of theme, 2: number of suggested plugins
				'nag_install_multiple' => __( '%1$s suggests installing %2$s plugins.', 'my-text-domain' ),
				'child_theme'          => false,
				'plugin_file'          => '',
			));
		...
		private function set_notices() {
			...
			/**
			 * Include _My_Plugin_Notices class to setup
			...
			$notices = new _My_Plugin_Notices( $args, $this, $this->plugins );
		...
		public function add_assets() {
			...
			wp_localize_script(
				...
				array(
					'thirdParty'   => __( 'Only plugins from wordpress.org can be installed directly here.', 'my-text-domain' ),
					'notInstalled' => __( 'Plugin update skipped because it is not installed.', 'my-text-domain' ),
				)
			);
		...
		public function display_page() {
			...
			<div id="suggested-plugins" class="wrap">
				...
				<?php else : ?>
					...
					<p><?php esc_html_e( 'No suggested plugins given.', 'my-text-domain' ); ?></p>
		...
		private function display_table_nav( $which ) {
			...
			$actions = array(
				'install-selected'    => __( 'Install', 'my-text-domain' ),
				'activate-selected'   => __( 'Activate', 'my-text-domain' ),
				'deactivate-selected' => __( 'Deactivate', 'my-text-domain' ),
				'update-selected'     => __( 'Update', 'my-text-domain' ),
				'delete-selected'     => __( 'Delete', 'my-text-domain' ),
			);
			...
			<div class="tablenav">
				<div class="actions bulkactions">
					...
					<label for="bulk-action-selector-<?php echo esc_attr( $which ); ?>" class="screen-reader-text">
						<?php esc_html_e( 'Select bulk action', 'my-text-domain' ); ?>
					</label>

					<select name="action-<?php echo $which; ?>" id="bulk-action-selector-<?php echo esc_attr( $which ); ?>">

						<option value="-1"><?php esc_html_e( 'Bulk Actions', 'my-text-domain' ); ?></option>
					...
					<?php
					submit_button( __( 'Apply', 'my-text-domain' ), 'action', '', false,
						array(
							'id' => "do-action-$which",
						)
					);
				...
				<tr>
					...
					<td id="cb" class="manage-column column-cb check-column">
						<label class="screen-reader-text" for="cb-select-all-<?php echo $id; ?>">
							<?php esc_html_e( 'Select All', 'my-text-domain' ); ?>
						</label>
						<input id="cb-select-all-<?php echo $id; ?>" type="checkbox">
					</td>

					<th scope="col" class="manage-column column-name column-primary">
						<?php esc_html_e( 'Plugin', 'my-text-domain' ); ?>
					</th>

					<th scope="col" class="manage-column column-compatible-version">
						<?php esc_html_e( 'Compatible Version', 'my-text-domain' ); ?>
					</th>

					<th scope="col" class="manage-column column-installed-version">
						<?php esc_html_e( 'Installed Version', 'my-text-domain' ); ?>
					</th>

					<th scope="col" class="manage-column column-status">
						<?php esc_html_e( 'Status', 'my-text-domain' ); ?>
					</th>
			...
			<tr class="<?php echo esc_attr( $class ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-plugin="<?php echo esc_attr( $plugin['file'] ); ?>" data-status="<?php echo esc_attr( $plugin['status'] ); ?>" data-source="<?php echo esc_attr( $this->get_plugin_source( $plugin ) ); ?>">
				...
				<th scope="row" class="check-column">
					<label class="screen-reader-text" for="">
						...
						printf( __( 'Select %s', 'my-text-domain' ), $plugin['name'] );
						?>
					</label>
				...
				<td class="column-installed-version">
					...
					<?php else : ?>
						<?php esc_html_e( 'Not Installed', 'my-text-domain' ); ?>
					<?php endif; ?>
				</td>
		...
		private function display_actions( $plugin ) {
			...
			if ( $is_wp || 'not-installed' !== $plugin['status'] ) {
				...
				$actions['details'] = array(
					'url'    => $plugin['url'],
					'text'   => __( 'Details', 'my-text-domain' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'More Information about %s', 'my-text-domain' ), $plugin['name'] ),
					'target' => '_blank',
					'nonce'  => null,
				);
			...
			if ( 'not-installed' === $plugin['status'] ) {
				...
				if ( $is_wp ) {
					...
					$actions['install'] = array(
						'url'    => $url,
						'text'   => __( 'Install', 'my-text-domain' ),
						// translators: 1: name of plugin
						'label'  => sprintf( __( 'Install %s', 'my-text-domain' ), $plugin['name'] ),
						'target' => '_self',
					...
					$actions['install'] = array(
						'url'    => $plugin['url'],
						'text'   => __( 'Get Plugin', 'my-text-domain' ),
						// translators: 1: name of plugin
						'label'  => sprintf( __( 'Install %s', 'my-text-domain' ), $plugin['name'] ),
						'target' => '_blank',
						'nonce'  => null,
			...
			if ( 'inactive' === $plugin['status'] ) {
				...
				$actions['activate'] = array(
					'url'    => $url,
					'text'   => __( 'Activate', 'my-text-domain' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Activate %s', 'my-text-domain' ), $plugin['name'] ),
					'target' => '_self',
				);
				...
				$actions['deactivate'] = array(
					'url'    => $url,
					'text'   => __( 'Deactivate', 'my-text-domain' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Deactivate %s', 'my-text-domain' ), $plugin['name'] ),
					'target' => '_self',
			...
			if ( 'inactive' === $plugin['status'] ) {
				...
				$actions['delete'] = array(
					'url'    => $url,
					'text'   => __( 'Delete', 'my-text-domain' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Delete %s', 'my-text-domain' ), $plugin['name'] ),
					'target' => '_self',
			...
			if ( $plugin['update'] ) {
				...
				$actions['update'] = array(
					...
					'text'   => sprintf(
						...
						__( 'Update to %s', 'my-text-domain' ),
						$plugin['new_version']
					),
					'label'  => sprintf(
						// translators: 1: name of plugin, 2: new version of plugin
						__( 'Update %1$s to version %2$s', 'my-text-domain' ),
						$plugin['name'],
						$plugin['new_version']
					),
					'target' => '_self',
				...
				$actions['update'] = array(
					'text' => __( 'Plugin is up-to-date', 'my-text-domain' ),
				);
		...
		private function display_status( $plugin ) {
			...
			switch ( $plugin['status'] ) {
				case 'active':
					esc_html_e( 'Active', 'my-text-domain' );
					break;

				case 'incompatible':
					esc_html_e( 'Incompatible', 'my-text-domain' );
					break;

				case 'inactive':
					esc_html_e( 'Installed', 'my-text-domain' );
					break;

				default:
					esc_html_e( 'Not Installed', 'my-text-domain' );
			}
		...
		public function request() {
			...
			if ( isset( $_REQUEST['success'] ) ) {
				...
				if ( 'activate' === $_REQUEST['action'] ) {
					...
					$message = __( 'Plugin activated.', 'my-text-domain' );
					...
					$message = __( 'Plugin deactivated.', 'my-text-domain' );

				} elseif ( 'deactivate-selected' === $_REQUEST['action'] ) {

					$message = __( 'Plugins deactivated.', 'my-text-domain' );
			...
			if ( $do_bulk && empty( $_REQUEST['checked'] ) ) {
				...
				$error = __( 'No plugins were selected.', 'my-text-domain' );

			} elseif ( ! $do_bulk && empty( $_REQUEST['plugin'] ) ) {

				$error = __( 'No plugin slug was given.', 'my-text-domain' );

			} elseif ( ! current_user_can( 'update_plugins' ) ) {

				$error = __( 'Sorry, you are not allowed to update plugins for this site.', 'my-text-domain' );

			} elseif ( ! $do_bulk && empty( $plugins_data[ $_REQUEST['plugin'] ] ) ) {

				$error = sprintf(
					// translators: 1: slug of plugin being activated
					__( 'The plugin %s doesn\'t exist within the plugin manager\'s registered plugins.', 'my-text-domain' ),
					$_REQUEST['plugin']
				);
			...
			if ( ! $error ) {
				...
				if ( $do_bulk ) {
					...
					if ( ! $plugins ) {
						$error = __( 'No valid plugins given for bulk action.', 'my-text-domain' );
					}
				} else {
				...
				if ( 'activate' === $_REQUEST['action'] ) {
					...
					if ( $success ) { // At least one plugin needed to be successful.
						...
						$error = __( 'None of the selected plugins could be activated. Make sure they are installed.', 'my-text-domain' );
